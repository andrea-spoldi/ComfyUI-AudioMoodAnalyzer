...
from fear_of_the_art_audio_analyzer.formatter_nodes import OllamaModelSelector
...
def test_returns_newline_separated_model_names():
    ...
    with patch("fear_of_the_art_audio_analyzer.formatter_nodes.requests.get", side_effect=_mock_get):
...
def test_strips_trailing_slash_before_appending_tags():
    ...
    with patch("fear_of_the_art_audio_analyzer.formatter_nodes.requests.get", side_effect=capturing_get):
...
def test_handles_empty_model_list():
    ...
    with patch("fear_of_the_art_audio_analyzer.formatter_nodes.requests.get", side_effect=empty_get):
...
def test_handles_connection_error():
    ...
    with patch("fear_of_the_art_audio_analyzer.formatter_nodes.requests.get", side_effect=ConnectionError("refused")):
        models_list, first_model = node.list_models("http://localhost:11434")
